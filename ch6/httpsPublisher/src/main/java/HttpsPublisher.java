...
public class HttpsPublisher {
    ...
    private HttpsServer getHttpsServer(String uri , int port) {
        ...
        try {
            ...
            server = HttpsServer.create(inet , HttpsPublisher.backlog);
            SSLContext sslCtx = SSLContext.getInstance("TLS");

            // password for keystore
            char[] password = "qubits".toCharArray();
            KeyStore ks = KeyStore.getInstance("JKS");
            //            FileInputStream fis = new FileInputStream(HttpsPublisher.keystore);
            InputStream fis =
                Thread.currentThread().getContextClassLoader().getResourceAsStream(HttpsPublisher.keystore);
    ...
    private void init(String serviceName , String uri) {
        try {
            ...
            this.serviceInstance = (IService) serviceClass.newInstance();
        } catch (Exception e) {
            throw new RuntimeException(e);
        }

        HttpsServer server = this.getHttpsServer(uri , HttpsPublisher.defaultPort);
        if (server != null) {
            server.createContext(uri);
            System.out.println("Server listening on port " + HttpsPublisher.defaultPort);
            server.start();
...
class MyHttpsHandler implements HttpHandler {
    ...
    public void handle(HttpExchange ex) {
        ...
        if (verb.equals("GET")) this.service.doGet(ex);
        else if (verb.equals("POST")) this.service.doPost(ex);
        else if (verb.equals("PUT")) this.service.doPut(ex);
        else if (verb.equals("DELETE")) this.service.doDelete(ex);

...
public class RestfulAmazon {
        ...
        try {
            ...
            for (int i = 0 ; i < results.getLength() ; i++) {
                ...
                for (int j = 0 ; j < nodes.getLength() ; j++) {
                    ...
                    if (child.getNodeType() == Node.TEXT_NODE) author = child.getNodeValue();
                }
            }
        } catch (Exception e) {
    ...
    private void lookupStuff(String accessKeyId , String secretKey) {
        RequestHelper helper = new RequestHelper(RestfulAmazon.endpoint , accessKeyId , secretKey);
        ...
        params.put("ItemId" , RestfulAmazon.itemId);
        params.put("ResponseGroup" , "Small");
        params.put("AssociateTag" , "kalin"); // any string should do

        requestUrl = helper.sign(params);
        String response = this.requestAmazon(requestUrl);
        ...
        System.out.println("Author: " + this.getAuthor(noNullResponse));
    }

...
public class AdagesApplication extends Application {
    ...
    public synchronized Restlet createInboundRoot() {
        ...
        // DELETE handler
        Restlet janitor = new Restlet(this.getContext()) {
            @Override
            public void handle(Request request , Response response) {
                String msg = null;

                String sid = (String) request.getAttributes().get("id");
                if (sid == null) msg = AdagesApplication.this.badRequest("No ID given.\n");

                Integer id = null;
                try {
                    id = Integer.parseInt(sid.trim());
                } catch (Exception e) {
                    msg = AdagesApplication.this.badRequest("Ill-formed ID.\n");
                }

                Adage adage = Adages.find(id);
                if (adage == null) msg = AdagesApplication.this.badRequest("No adage with ID " + id + "\n");
                else {

...
public class AdagesProvider implements Provider<Source> {
    ...
    private StreamSource adage2Xml(Adage adage) {
        String str = this.toXml(adage);
        return this.toSource(str);
    }

    private StreamSource adages2Xml() {
        String str = this.toXml(Adages.getListAsArray());
        return this.toSource(str);
    }
    ...
    private Source doDelete(MessageContext mctx) {
        ...
        if (qs == null) throw new HTTPException(403); // illegal operation
            ...
            return this.toSource(this.toXml(msg));
        }
    }
    ...
    private Source doGet(MessageContext mctx) {
        ...
        // Get all Adages.
        if (qs == null) return this.adages2Xml();
    ...
    private Source doPost(Source request) {
        ...
        InputSource in = this.toInputSource(request);

        String pattern = "//words/text()"; // find the Adage's "words"
        String words = this.findElement(pattern , in);
    ...
    private Source doPut(Source request) {
        ...
        if (parts[0].length() < 1 || parts[1].length() < 1) throw new HTTPException(400); // bad request
    ...
    public Source invoke(Source request) {
        if (this.wctx == null) throw new RuntimeException("Injection failed on wctx.");
        ...
        if (httpVerb.equals("GET")) return this.doGet(mctx);
        else if (httpVerb.equals("POST")) return this.doPost(request);
        else if (httpVerb.equals("PUT")) return this.doPut(request);
        else if (httpVerb.equals("DELETE")) return this.doDelete(mctx);

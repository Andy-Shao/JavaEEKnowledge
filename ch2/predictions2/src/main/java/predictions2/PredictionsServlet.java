...
public class PredictionsServlet extends HttpServlet {
    ...
    public void doDelete(HttpServletRequest request , HttpServletResponse response) {
        ...
        try {
            this.predictions.getMap().remove(key);
            String msg = "Prediction " + key + " removed.\n";
            this.sendResponse(response , this.predictions.toXML(msg) , false);
        } catch (Exception e) {
    ...
    public void doGet(HttpServletRequest request , HttpServletResponse response) {
        ...
        if (key == null) {
            ConcurrentMap<Integer , Prediction> map = this.predictions.getMap();

            // Sort the map's values for readability.
            Object[] list = map.values().toArray();
            Arrays.sort(list);

            String xml = this.predictions.toXML(list);
            this.sendResponse(response , xml , json);
        }
        // Otherwise, return the specified Prediction.
        else {
            Prediction pred = this.predictions.getMap().get(key);

            if (pred == null) { // no such Prediction
                String msg = key + " does not map to a prediction.\n";
                this.sendResponse(response , this.predictions.toXML(msg) , false);
            } else this.sendResponse(response , this.predictions.toXML(pred) , json);
        }
    }
    ...
    public void doPost(HttpServletRequest request , HttpServletResponse response) {
        ...
        int id = this.predictions.addPrediction(p);
    ...
    public void doPut(HttpServletRequest request , HttpServletResponse response) {
        ...
        Prediction p = this.predictions.getMap().get(new Integer(key.trim()));
        if (p == null) { // not found? 
            String msg = key + " does not map to a Prediction.\n";
            this.sendResponse(response , this.predictions.toXML(msg) , false);
        } else if (rest == null) throw new HTTPException(HttpServletResponse.SC_BAD_REQUEST);
        else {

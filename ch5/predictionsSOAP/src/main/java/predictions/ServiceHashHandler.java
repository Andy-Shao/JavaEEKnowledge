...
public class ServiceHashHandler implements SOAPHandler<SOAPMessageContext> {
    ...
    public boolean handleMessage(SOAPMessageContext mCtx) {
        ...
        if (!outbound) try {
            ...
            if (soapHeader == null) this.generateFault(msg , "No header!");

            Node node = soapHeader.getFirstChild(); // credentials
            NodeList nodeList = node.getChildNodes(); // Name, Timestamp, Signature
            if (nodeList.getLength() < 3) this.generateFault(msg , "Too few header blocks!");
            ...
            if (name == null || timestamp == null || signature == null) this.generateFault(msg ,
            ...
            if (secret == null) this.generateFault(msg , name + " not registered!");

            byte[] secretBytes = this.getBytes(secret);
            String localSignature = this.getSignature(name , timestamp , secretBytes);

            if (!this.verify(signature , localSignature)) this.generateFault(msg , "HMAC signatures do not match.");
        } catch (Exception e) {
